...
CREATE TABLE `blogs` (
  ...
  PRIMARY KEY (`blog_id`),
  KEY `blog_by` (`blog_by`),
  CONSTRAINT `Blogs-IfDeleteUser` FOREIGN KEY (`blog_by`) REFERENCES `users` (`idUsers`) ON DELETE CASCADE
...
CREATE TABLE `blogvotes` (
  ...
  KEY `voteBy` (`voteBy`),
  CONSTRAINT `blogvotes_ibfk_2` FOREIGN KEY (`voteBy`) REFERENCES `blogs` (`blog_by`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TRIGGER `calc_blog_votes_after_insert` AFTER INSERT ON `blogvotes` FOR EACH ROW
BEGIN
...
CREATE TRIGGER `calc_blog_votes_after_update` AFTER UPDATE ON `blogvotes` FOR EACH ROW
BEGIN
...
CREATE TRIGGER `calc_blog_votes_after_delete` AFTER DELETE ON `blogvotes` FOR EACH ROW
BEGIN
...
CREATE TABLE `event_info` (
  ...
  KEY `event` (`event`),
  KEY `title` (`title`),
  CONSTRAINT `event_info_ibfk_1` FOREIGN KEY (`event`) REFERENCES `events` (`event_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `polls` (
  ...
  CONSTRAINT `Polls-IfDeleteUser` FOREIGN KEY (`created_by`) REFERENCES `users` (`idUsers`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(210,	'asadads',	'2022-10-21 08:48:56',	56,	52,	0);
...
CREATE TRIGGER `calc_forum_votes_after_insert` AFTER INSERT ON `postvotes` FOR EACH ROW
BEGIN
...
CREATE TRIGGER `calc_forum_votes_after_update` AFTER UPDATE ON `postvotes` FOR EACH ROW
BEGIN
...
CREATE TRIGGER `calc_forum_votes_after_delete` AFTER DELETE ON `postvotes` FOR EACH ROW
BEGIN
...
CREATE TABLE `reports` (
  ...
  CONSTRAINT `reports_ibfk_1` FOREIGN KEY (`report_by`) REFERENCES `users` (`idUsers`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

INSERT INTO `reports` (`report_id`, `report_by`, `report_date`, `link`, `description`, `message`) VALUES
(2,	61,	'2022-10-26',	'https://lazatech.tech/posts.php?topic=53',	'rehrth',	'dsvdb');
...
CREATE TABLE `topics` (
  ...
  CONSTRAINT `topics_ibfk_2` FOREIGN KEY (`topic_by`) REFERENCES `users` (`idUsers`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(40,	2,	'Gabriel Edrian',	'Alvaro',	'Gabby',	'gabrielalvaro693@gmail.com',	'$2y$10$9x.hO88k83A73U.Emxf.AeYyombRMUG8j7xap0knPMOaNTjQ/cWp.',	'm',	'@SeulGabi',	'',	'6197b0bfb0f292.02584410.jpg'),
...
(51,	2,	'Marian',	'Guerra',	'marianzg1015',	'marian.guerra@g.batstate-u.edu.ph',	'$2y$10$yJn581/GMJeww6ajrtKfIuZ0YtSGhlUjYdazet9.DV268f/eqCJhK',	'f',	'Future Graduate❤️',	'',	'624451d33535b5.69181310.jpg'),
(52,	2,	'Mariel',	'Guerra',	'MARIEL',	'mariel.guerra@g.batstate-u.edu.ph',	'$2y$10$DjTEP57v5l5UizTGvtVbDe872VB/JhxwWOS1DhImSULMyGxORc5Be',	'f',	'\"Ad meliora.\"',	'... ',	'624454436e1c52.21706905.png'),
(53,	0,	'',	'',	'MarielZG',	'marielzguerra09@gmail.com',	'$2y$10$WtKQjhBviFTesVY3sl8Puujhhbolf2SUyMYy3E8mr.8iIE/9dzWaa',	'm',	'',	'',	'default.png'),
(55,	0,	'Gabriel Edrian',	'Alvaro',	'TrialMe',	'gabrielalvaro693@gmail.com',	'$2y$10$niIyGjIeldO0eD.BHf9SkeQtoOnuyOhCycC4/xFPenbHxDvKEsDuy',	'm',	'',	'',	'default.png'),
(57,	0,	'Error',	'Checkers',	'Error',	'errorcheckers@gmail.com',	'$2y$10$S.6jFdYkzmYTHSfXygVlVONDyIslosT.xszjRrViZjUClwTxIsKn2',	'm',	'',	'',	'default.png'),
(58,	0,	'Kaye',	'Perez',	'Kaye',	'hannahkayeperez@gmail.com',	'$2y$10$7rXEWAb7v4dqNRlPCtyQaOToazI.29q52Sl3cZY5/Yio6AO0BA5UC',	'f',	'',	'Pretty. Cute',	'default.png'),
(61,	1,	'Lazatech',	'Organization',	'Lazatech',	'lazatech.org@gmail.com',	'$2y$10$TFR8jPKquUfmXG.v4DNpT.7YQZlqJXTiFNeSjaMtLT2zAy/293Jka',	'm',	'@LazatechS',	'',	'635bd367e48681.81860864.jpg'),
(62,	3,	'Marian',	'Guerra',	'marianzg111',	'marianzguerra09@gmail.com',	'$2y$10$tklFasUYTOLfbg0we8BNp.nDmYKhvdPy5cw.2MjlyZLrvvAAgk91y',	'm',	'',	'',	'default.png');
